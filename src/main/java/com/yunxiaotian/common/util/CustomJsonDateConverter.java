...
public class CustomJsonDateConverter extends ObjectMapper {
	public CustomJsonDateConverter() {
		...
		// oracle.sql.BLOB
		factory.addGenericMapping(oracle.sql.BLOB.class, new JsonSerializer<oracle.sql.BLOB>() {
			@Override
			public void serialize(BLOB value, JsonGenerator jgen, SerializerProvider provider)
				...
				try {
					...
					if (is == null) {
						...
						int lenth = (int) value.length();
						bit = new byte[lenth];
						is.read(bit);
						jgen.writeBinary(bit);
					}
				} catch (Exception e) {
					jgen.writeString("BLOB EXCEPTION");
				} finally {
					if (is != null) {
						try {
							is.close();
